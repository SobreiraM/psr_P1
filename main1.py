...
#Inicialização
def Start_Test():
    print('Pressionar qualquer tecla para iniciar o teste\n')
        
    Key_pressed = readchar.readkey()#Key_pressed -> caracter pressionado para iniciar o teste!!
    if Key_pressed == chr(32):  #Pressionada a tecla espaço   
        print( Back.RED + '\nTeste interrompido!' + Style.RESET_ALL)
        exit()  #Saida do programa
    else:
        print( Back.GREEN + 'Inicio do teste!' + Style.RESET_ALL)
        sleep(1)
...
##############################################
        if letter == chr(32): #Pressionada a tecla espaço                  
                print( Back.RED + '\nTeste interrompido!' + Style.RESET_ALL)
                #t_end = time()
                #print('Current test duration (' + Fore.CYAN + (str(t_end - t_start)) + Style.RESET_ALL + ") exceeds maximum of " + str(segundos))
                #Statistics(t_start, t_end)
...
def main():
    ...
    if args['use_time_mode']:
        print("Calling time mode")                  #debug, tirar quando n for necessário 
        total_time = args['maximum_value']
        if args['use_word_mode']:
            print('calling word mode')          #debug, tirar quando n for necessário 
            Start_Test()
            Time_Words(args['maximum_value'])
                
        else:
            print('calling letter mode')        #debug, tirar quando n for necessário 
            Start_Test()
            Time_Letters(args['maximum_value'])

                    
    else:
        print("Calling Input")                   #debug, tirar quando n for necessário 
